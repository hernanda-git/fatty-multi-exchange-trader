...
def test_classifier_preserves_only_values_present_in_json() -> None:
    result = classify_json(
        ...
        json.dumps({
            "actionable": True, "pair": "PYTH", "side": "LONG", "entry": 0.0568,
            "stop_loss": 0.05462, "take_profits": [0.0708], "confidence": 0.91,
...
def test_classifier_does_not_invent_missing_trade_geometry() -> None:
    result = classify_json(
        "$ETH 6% up", json.dumps({"actionable": False, "reason": "movement only"}),
        message_id=16084,
    )
...
def test_classifier_rejects_invalid_canonical_geometry() -> None:
    result = classify_json(
        "BTC LONG", json.dumps({
            "actionable": True, "pair": "BTC", "side": "LONG", "entry": 100,
            "stop_loss": 101, "take_profits": [110], "confidence": 0.8, "reason": "bad",
        }), message_id=3,
    )
    assert result.actionable is False
    assert result.signal is None
